          ...
          <div className="grid gap-10 pb-10 md:grid-cols-2 lg:grid-cols-4">
            <div>
              <Link to="/" className="flex items-center gap-3 text-white no-underline">
                <span className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-[12px] border border-white/20 bg-white p-1 shadow-sm">
              ...
              <div className="mt-5">
                <p className="text-xs font-extrabold tracking-[0.16em] text-white/55 uppercase">
                  Accreditations
                </p>
                <ul className="mt-3 flex flex-wrap items-center gap-3">
                  {school.accreditations.map((body) => (
                    <li key={body.id}>
                      <img
                        src={body.src}
                        alt={`${body.name} (${body.shortName})`}
                        className="h-12 w-auto max-w-[9rem] rounded-md bg-white object-contain p-1"
                        loading="lazy"
                        decoding="async"
                      />
                    </li>
            ...
            <div className="lg:pl-8">
              ...
              <ul className="mt-4 space-y-2 text-sm text-white/75">
                {usefulLinks.map((link) => (
                  <li key={link.label}>
                    <Link to={link.to} className="hover:text-[var(--orange-bright)]">
                      {link.label}
                    </Link>
                  </li>
                ))}
                ...
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    ...
                    <a
                      ...
                      className="text-white/70 hover:text-[var(--orange-bright)]"
                    >
